 ...
 *       tagged w/bthom
 *
 * Tue Jun 29 2010, Christopher Mitchell, modified for use with
 *      Piccolo project for Courant::NEWS
 *
 ******************************************************************
 */

#define NEURONS_HIDDEN 256
...
void backprop_face(IMAGELIST *trainlist,
          IMAGELIST *test1list, IMAGELIST *test2list,
          int epochs, int savedelta, char* netname, int hidden_neurons,
          int list_errors);
void printusage();
...
int main(int argc, char* argv[] )
{
  ...
  savedelta = 100;
  list_errors = 0;
  netname[0] = infopn[0] = pathpn[0] = '\0';
  hiddenn = NEURONS_HIDDEN;
    ...
    if (argv[ind][0] == '-') {
      switch (argv[ind][1]) {  
        ...
        case 'S': savedelta = atoi(argv[++ind]);
                  break;
		case 'p': 
				  strcpy(pathpn, argv[++ind]);
                  break;
        case 'i':
				  strcpy(infopn, argv[++ind]);
                  break;
		case 'h':
                  hiddenn = atoi(argv[++ind]);
                  break;
  ...
  if (infopn[0] == '\0' || pathpn[0] == '\0') {
    printf("%s: Must specify all of -p, -i!\n",argv[0]);
    exit(-1);
  }
  ...
  /*** If any train, test1, or test2 sets have been specified, then
       load them in. ***/
  if (infopn[0] != '\0') 
    imgl_load_images_from_infofile(trainlist, pathpn, infopn);
//  if (test1name[0] != '\0') 
//    imgl_load_images_from_infofile(test1list, fpath, test1name);
//  if (test2name[0] != '\0')
//    imgl_load_images_from_infofile(test2list, fpath, test2name);
...
void backprop_face(trainlist, test1list, test2list, epochs, savedelta, netname,
	      hidden_neurons,list_errors)
IMAGELIST *trainlist, *test1list, *test2list;
int epochs, savedelta, hidden_neurons, list_errors;
char *netname;
{
  IMAGE *iimg;
  BPNN *net;
  ...
  if ((net = bpnn_read(netname)) == NULL) {
	...
	make a net with:
	  imgsize inputs, 4 hiden units, and 1 output unit
          */
      net = bpnn_create(imgsize, hidden_neurons, 1);
    } else {
      printf("Need some images to train on, use -ip and -in\n");
      return;
    }
  }
...
void performance_on_imagelist(net, il, list_errors)
BPNN *net;
IMAGELIST *il;
int list_errors;
{
...
char *prog;
{
  ...
  printf("       -pp <path to positive samples>\n");
  printf("       -ip <filename of positive sample list>\n");
  printf("       -pn <path to negative samples>\n");
  printf("       -in <filename of negative sample list>\n");
  printf("       [-h <hidden nodes, 256 by default>\n");
  printf("       [-T]\n");
}
